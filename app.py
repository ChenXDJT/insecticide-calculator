...
def main():
    ...
    if is_indoor:
        ...
        known_type = "dilution"
        known_dilution = st.number_input("稀释倍数数值 *", min_value=0.0, value=1.0, step=1.0)
    else:
        # 室外场景：用户选择已知条件类型
    ...
    if calc_btn:
        # 将中文药剂类型转换为英文标识符
        chem_type_en = "active" if chem_type == "有效成分用药量" else "formulation"

        # 参数校验
        errors = validate_parameters(
            flow_rate, swath_width, fog_height, space_val,
            chem_type_en, dosage_value, active_dosage, active_conc,
        ...
        if errors:
            ...
            try:
                ...
                # 构建空间参数对象（显式传入输入类型，不再依赖字符串解析）
                input_is_volume = "空间" in space_type
                space = SpaceParameter(space_val, H, input_is_volume, is_indoor)

                # 构建药剂参数对象
                chemical = ChemicalParameter(
                    chem_type=chem_type_en,
                ...
                # 确定计算模式（体积法/面积法）
                # 有效成分模式锁定为 ml/m³，制剂模式使用用户选择
                if chem_type == "有效成分用药量":
                    dosage_unit_internal = "ml/m³"
                else:
                    dosage_unit_internal = dosage_unit
                
                calc_by_volume = (dosage_unit_internal == "ml/m³")

                # 调用求解引擎
                if known_type == "speed":
                    v_ms = convert_speed(known_speed, speed_unit)
                    result = solve_by_speed(Q, W, H, space, chemical, v_ms, calc_by_volume)
                else:
                    result = solve_by_dilution(
                        Q, W, H, space, chemical,
                        known_dilution, calc_by_volume, is_indoor
                    )
